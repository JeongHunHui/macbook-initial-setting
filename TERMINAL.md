...
`prefix2 None`을 명시해 재로드 때도 `Ctrl+A`를 입력기에 전달한다. 설정에서 옛 줄을 지우는 것만으로는 실행 중인 tmux의 보조 prefix가 꺼지지 않는다. `Ctrl+L`에 추가돼 있던 tmux 스크롤 기록 삭제도 해제한다.
...
`⌘K` 화면 지우기, `⌘J` 선택 위치 점프, `⌘Enter` 전체화면, `⇧⌘Enter` 분할 확대, 분할 이동·크기 변경, 화면 내용을 입력창에 붙이는 단축키도 차단한다. 복사·붙여넣기, 검색, 새 창·탭, 일반 닫기, 글꼴 크기, macOS 표준 전체화면(`⌃⌘F`)은 유지한다. Ghostty의 **Reload Configuration** 메뉴로 실행 중인 창에도 적용할 수 있다.
...
1. **Karabiner-Elements** 를 한 번 실행하고 시스템 설정 > 개인정보 보호 및 보안에서 입력 모니터링, 손쉬운 사용, 드라이버 확장을 허용한다.
